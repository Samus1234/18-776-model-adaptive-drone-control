# -*- coding: utf-8 -*-
"""
Created on Sun Dec 12 23:23:58 2021
...
class PID(object):
    ...
    def attitudePID(self):
        xddot = self.ref[6:][0] + self.Kv_x*(self.ref[3:][0] - self.x[1]) + self.Kp_x*(self.ref[0] - self.x[0]) - self.x[12]
        yddot = self.ref[6:][1] + self.Kv_y*(self.ref[3:][1] - self.x[3]) + self.Kp_y*(self.ref[1] - self.x[2]) - self.x[13]
        zddot = self.u[0]/m + g
        psi_ref = self.ref[-1]
...
Q = np.diag([1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-1, 1e-1, 1e-1])
...
def fs(t, X):
    ...
    X_dot[::2] = X[1::2]
    X_dot[1::2][:3] = (1/m) * (-np.matmul(A_drag, X[1::2][:3]) + Q_trans(X)) + np.array([sin(2*pi*t/50)*5, 0, 0])/m
...
def f_est(x, u):
    return x + np.hstack((Ts*(fs(0, x[:-3]) + np.matmul(gs(x[:-3]), u) + np.array([0, x[12], 0, x[13], 0, x[14], 0, 0, 0, 0, 0, 0])/m), np.zeros(3)))
...
def plotControlEffort():
    ...
    plt.plot(t, sin(2*pi*t/50)*5 + np.random.normal(0, 0.01, N), label = 'Wind Force in Newtons', lw = 2.5)
    plt.plot(t, X_hat[:, 12], label = 'Estimated Wind Force in Newtons', lw = 2.5)
    plt.xlabel('time in seconds', fontsize = 18)
    plt.ylabel('Wind Disturbance in N', fontsize = 18)
    plt.legend(fontsize = 18)
...
ani.save('droneAdaptiveNDI.gif', writer='imagemagick')